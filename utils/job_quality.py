...
def normalize_posting_date(value: str) -> tuple[str, str]:
    """Return (ISO-ish normalized value, freshness bucket)."""
    ...
    if m:
        ...
        bucket = "FRESH" if age_days <= 1 else "RECENT" if age_days <= 7 else "STALE"
    ...
    for fmt in ("%Y-%m-%d", "%Y/%m/%d", "%d-%m-%Y"):
        try:
            ...
            bucket = "FRESH" if age_days <= 1 else "RECENT" if age_days <= 7 else "STALE"
            return dt.date().isoformat(), bucket
        except ValueError:
            continue
...
def description_quality(listing: JobListing) -> int:
    ...
    score += min(30, sum(5 for keyword in keywords if keyword in desc))
...
def _job_fingerprint(listing: JobListing) -> str:
    ...
    payload = "|".join([
        normalize_company(listing.company),
        normalize_title(listing.title),
        normalize_location(listing.location),
        normalize_text(listing.description)[:1000],
    ])
...
def deduplicate_jobs(listings: list[JobListing]) -> tuple[list[JobListing], int]:
    """Collapse exact and strong cross-source duplicates, keeping the best record."""
    ...
    for members in groups.values():
        best = max(members, key=data_quality_score)
        merged.append(best)

    # Second pass handles syndicated jobs that use different URLs.
    ...
    for listing in merged:
        ...
        if existing is None:
            ...
            continue
        # Require strong description similarity to avoid collapsing distinct openings.
        ...
        if similarity >= 0.35 or not existing.description or not listing.description:
            identity_collisions += 1
            if data_quality_score(listing) > data_quality_score(existing):
                by_identity[key] = listing
        else:
            unique_key = key + f"|{listing.job_url}"
...
def relevance_score(listing: JobListing) -> float:
    """Deterministic pre-AI ranking using profile-relevant signals only, never company prestige."""
    ...
    preferred_locations = ("bengaluru", "pune", "hyderabad", "gurugram", "gurgaon", "noida", "mumbai", "chennai", "remote", "india")
