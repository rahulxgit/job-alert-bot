...
The controller computes a safe first-pass candidate budget from the configured
AI deadline, worker concurrency, historical latency and the previous run's
unresolved rate. It never exceeds MAX_LLM_CANDIDATES and never silently drops
jobs: candidates beyond the admitted budget remain in the existing pipeline's
unprocessed set for the next run/checkpoint flow.
"""
...
def compute_admission_limit(metrics: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # A high unresolved rate is evidence that the configured capacity is too
    # aggressive for the current provider/runtime conditions. Reduce admission
    # proportionally but keep a usable floor so a degraded run can still make
    # progress.
...
def apply_admission_control() -> int:
    ...
    if github_env:
        with open(github_env, "a", encoding="utf-8") as handle:
            handle.write(f"MAX_LLM_CANDIDATES={admitted}\n")

    print(
        "AI admission control: "
        f"admitting {admitted}/{decision['configured_max_candidates']} candidates; "
